...
class TestDataBase(TestCase):
    ...
    def test_check_operation_data_type_works(self):
        self.assertEqual(DataBase().check_operation_data_type(), "")
        self.assertEqual(DataBase().check_operation_data_type("a","b"), " WHERE a = 'b'")
        with self.assertRaises(Exception):
            DataBase.check_operation_data_type(1, 2)
    ...
    def test_delete_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        ...
        with mock.patch("app.database.DataBase.cursor", create=True) as mock_cursor:
            mock_cursor.execute.side_effect = [0, 1]
            self.assertFalse(DataBase().delete_data("a"))
            self.assertTrue(DataBase().delete_data("a"))

        self.assertFalse(DataBase().delete_data("a"))

    @mock.patch("app.database.DataBase.is_database_selected")
    @mock.patch("app.database.DataBase.conn_and_cursor_exist")
    def test_list_data_works(self, mock_conn_and_cursor_exist, mock_is_database_selected):
        mock_conn_and_cursor_exist.return_value = False
        with self.assertRaises(Exception) as error:
            DataBase().list_data("a")
